...
由于 $f(\mathbf{x})$ 是通过 $\text{sigmoid}(x)$ 获取概率的，这导致输出不匹配概率的特性，即：
...
多层感知机相对于单个感知机存在着巨大的结构差异，原本在处理单个感知机时，我们不需要考虑跨层传递误差。而现在面对多层感知机，我们需要解决 **误差如何传递** 的问题。

我们假设层数是 $N$ ，则每层的输出为 $f_{n}(\mathbf{x})$ ，经过激活函数后为 $z_{n}$ 。

因此，第一层可以表示为：

$$
\begin{align*}
    f_{1}(\mathbf{x}) &= \mathbf{w}_{1} \mathbf{x} + b_{1} \\
    z_{1} &= \text{Leaky ReLU}(f_{1}(\mathbf{x}))
\end{align*}
$$
...
$$
\begin{align*}
    f_{2}(\mathbf{x}) &= \mathbf{w}_{2} \mathbf{z_{1}} + b_{2} \\
    z_{2} &= \text{Leaky ReLU}(f_{2}(\mathbf{x}))
\end{align*}
$$

以此类推我们可以表示其他层。

我们以一个三层的模型为例讲解如何进行优化。

首先观察上面的定义我们可以得知输入 $\mathbf{x}$ 后，在MLP中的传递路径：

$$
\mathbf{x} → f_{1}(\mathbf{x}) → \text{Leaky ReLU} → f_{2}(\mathbf{z_{1}}) → \text{Leaky ReLU} → \text{...} → \mathbf{y_{pred}}
$$
